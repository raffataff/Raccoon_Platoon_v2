...
class Game {
    constructor(canvasId) {
        ...
        this.selectedUnits = [];
        this.visualEffects = [];
        this.preloadedImages = {}; // To store loaded image objects
    ...
    async preloadLevelAssets() {
        ...
        const imagePromises = [];
        // this.preloadedImages is already a class member, initialized in constructor

        console.log("[Game] Preloading level assets...");

        // Preload Obstacle Sprites (existing logic)
        obstacleDefs.forEach(def => {
            const spritesToLoad = [];
            if (def.spriteNormal) spritesToLoad.push({ path: def.spriteNormal, key: def.spriteNormal });
            if (def.spriteDestroyed) spritesToLoad.push({ path: def.spriteDestroyed, key: def.spriteDestroyed });
            ...
            spritesToLoad.forEach(spriteInfo => {
                if (spriteInfo.path && !this.preloadedImages[spriteInfo.key]) {
                    const promise = new Promise((resolve) => {
                        ...
                        img.onerror = () => { console.error(`Failed to load asset: ${spriteInfo.path}`); this.preloadedImages[spriteInfo.key] = null; resolve(); };
        ...
        // --- NEW: Preload Grass Sprites ---
        const grassSpriteFiles = CONFIG.GRASS_SPRITE_FILES || [];
        const grassSpritePath = CONFIG.GRASS_SPRITE_PATH || 'assets/images/objects/grass/';
        grassSpriteFiles.forEach(fileName => {
            const fullPath = grassSpritePath + fileName;
            if (!this.preloadedImages[fullPath]) { // Check if already loaded/attempted
                const promise = new Promise((resolve) => {
                    const img = new Image();
                    img.onload = () => {
                        this.preloadedImages[fullPath] = img;
                        // console.log(`Loaded grass: ${fullPath}`);
                        resolve();
                    };
                    img.onerror = () => {
                        console.error(`Failed to load grass asset: ${fullPath}`);
                        this.preloadedImages[fullPath] = null; // Mark as failed
        ...
        console.log("[Game] Level assets preloading complete.");
    }


    start() { // No async needed here if preload is called before confirmSquadAndStartMission
        if (!this.masterRoster || this.masterRoster.length === 0) {
            console.error("Campaign not initialized before starting.");
            this.gameState = 'MAIN_MENU';
            if (this.ui) this.ui.showMainMenuScreen();
            return;
        }
        if (this.getAvailableRecruits().length === 0) {
            this.gameState = 'GAME_OVER_NO_RECRUITS';
            if(this.ui) this.ui.showGameOverScreen(CONFIG.UI_TEXT_STRINGS.ERROR_NO_INITIAL_RECRUITS);
            return;
         }
        if (this.loadMissionData(0, 0)) {
            if (this.ui && this.campaignData && this.campaignData[this.currentPhaseIndex] && this.currentMissionParams) {
                 this.ui.showPreMissionScreen_RecruitSelect(
                     this.campaignData[this.currentPhaseIndex], this.currentMissionParams, this.getAvailableRecruits()
                 );
            ...
            this.gameState = 'MAIN_MENU'; if (this.ui) this.ui.showMainMenuScreen();
            alert(CONFIG.UI_TEXT_STRINGS.ERROR_LOAD_FIRST_MISSION_FAILED);
        }
    }

    async confirmSquadAndStartMission(selectedRecruitsForDeployment) { // Made async
        const maxSquadSize = CONFIG.MAX_SQUAD_SIZE_MVP || 4;
        // ... (validation logic) ...
        if (!selectedRecruitsForDeployment || selectedRecruitsForDeployment.length === 0 ||
            selectedRecruitsForDeployment.length > maxSquadSize) {
            // ... (alert logic using CONFIG.UI_TEXT_STRINGS) ...
            ...
            if (!selectedRecruitsForDeployment || selectedRecruitsForDeployment.length === 0) {
                alertMsg = CONFIG.UI_TEXT_STRINGS.NO_RECRUITS_SELECTED_ALERT || "Select at least one Raccoon for the mission!";
            } else if (selectedRecruitsForDeployment.length > maxSquadSize) {
                 alertMsg = (CONFIG.UI_TEXT_STRINGS.MAX_SQUAD_ALERT || "Max squad size is {MAX_SQUAD_SIZE}. Please deselect some recruits.").replace('{MAX_SQUAD_SIZE}', maxSquadSize.toString());
            }
            alert(alertMsg);
            if (this.ui && this.campaignData && this.campaignData[this.currentPhaseIndex] && this.currentMissionParams) {
        ...
        this.gameState = 'LOADING_MISSION'; // Example new state
        if (this.ui && typeof this.ui.showLoadingScreen === 'function') { // You'd create this UI method
            this.ui.showLoadingScreen("Preparing battlefield...");
        } else {
            console.log("Loading mission..."); // Fallback log
        }

        await this.preloadLevelAssets(); // Wait for assets to load

        // --- Resume original logic after preloading ---
        ...
        this.deployedSquadRoster.forEach(r => {
            ...
            r.grenadeAmmo = startGrenades;
            r.isMoving = false; r.manualTarget = null; r.autoTarget = null;
            r.actionTimer = 0; r.isAimingGrenade = false;
        });

        this.gameState = 'RUNNING'; // Set to RUNNING after loading and setup
        this.isObjectiveComplete = false;
        this.missionStartedAndPopulated = false;
        this.fallenRaccoonsThisMission = [];
        ...
        // Pass the preloaded images map to the level generator
        const playerSpawnLocations = this.level.generateLevelAndGetPlayerSpawns(
            ...
            this.preloadedImages // Pass the map of loaded Image objects
        );
        ...
        this.deployedSquadRoster.forEach((raccoon, index) => {
            ...
            if (playerSpawnLocations[index]) {
                raccoon.x = playerSpawnLocations[index].x; raccoon.y = playerSpawnLocations[index].y;
                raccoon.targetX = raccoon.x; raccoon.targetY = raccoon.y; raccoon.game = this;
            } else {
                console.warn(`No spawn location for Raccoon ${index}. Fallback.`);
                raccoon.x = 100 + index * (CONFIG.RACCOON_SIZE * 3);
                raccoon.y = (CONFIG.WORLD_HEIGHT || 600) / 2;
            }
        });
        this.selectedUnits = [...this.deployedSquadRoster];
        this.gameObjects = []; this.visualEffects = [];
        this.isDragging = false; this.draggedFarEnough = false;

        if (this.deployedSquadRoster.length > 0) {
            // ... (camera setup) ...
            let avgX = 0, avgY = 0; this.deployedSquadRoster.forEach(unit => { avgX += unit.x; avgY += unit.y; });
            avgX /= this.deployedSquadRoster.length; avgY /= this.deployedSquadRoster.length;
            this.cameraX = avgX - this.canvas.width / 2; this.cameraY = avgY - this.canvas.height / 2;
            this.clampCamera();
        } else {
            this.cameraX = (CONFIG.WORLD_WIDTH - this.canvas.width) / 2; this.cameraY = (CONFIG.WORLD_HEIGHT - this.canvas.height) / 2;
            this.clampCamera();
        }

        // Optional: Hide loading indicator
        if (this.ui && typeof this.ui.hideLoadingScreen === 'function') {
            this.ui.hideLoadingScreen();
        }

        if (this.ui) {
            this.ui.hidePreMissionScreen(); this.ui.showHUD();
        ...
        if (!this.lastTime || this.gameState !== 'RUNNING') { // Should be 'RUNNING' now
            this.lastTime = performance.now();
            // gameLoop is already running, just ensure lastTime is reset for deltaTime
        }
        this.lastTime = performance.now(); // Reset for first accurate deltaTime
    }

    // ... (Rest of Game class methods: handle clicks, initializeNewCampaign, getAvailableRecruits, resizeCanvas, clampCamera, loadMissionData, recordRaccoonFallen, addNewRecruitToMasterRoster, endMission, proceedToNextLogicalStep, toggleFormation, setFormationSpacing, selectUnitsInDragRectangle, deselectAllUnits, selectAllPlayerUnits, addProjectile, addVisualEffect, checkMissionStatus, update, render, gameLoop, calculateFormationPoints)
    // (These methods were provided in full previously and mostly only had CONFIG value replacements, no major structural changes for this step)
    // Ensure all previously provided full methods are used.
    handlePrimaryLeftClick(worldX, worldY) {
        ...
        if (aimingRaccoons.length > 0) {
            ...
            } else if (clickedOnEnemyUnitForGrenade) {
                leaderAimer.moveToGrenadeRange(clickedOnEnemyUnitForGrenade);
            }
            if(this.inputHandler) this.inputHandler.updateMouseCursor();
            return;
        }
        ...
        if (this.deployedSquadRoster) {
             for (const unit of this.deployedSquadRoster) {
                if (unit.isAlive() && distance(worldX, worldY, unit.x, unit.y) < unit.size + 7) {
                    clickedOnPlayerUnit = unit;
                    break;
                }
            }
        }
        ...
        if (clickedOnPlayerUnit) {
            if (this.selectedUnits.length !== 1 || this.selectedUnits[0] !== clickedOnPlayerUnit) {
    ...
    handleShiftFireAtPointCommand(worldX, worldY) {
        if (this.gameState !== 'RUNNING' || !this.selectedUnits || this.selectedUnits.length === 0) {
            return;
        }

        const aimingRaccoons = this.selectedUnits.filter(
            unit => unit instanceof Raccoon && unit.isAimingGrenade && unit.isAlive()
        );
        if (aimingRaccoons.length > 0) {
            const leaderAimer = aimingRaccoons[0];
        ...
        this.selectedUnits.forEach(unit => {
            if (unit.isAlive() && typeof unit.fireAtPoint === 'function') {
                unit.fireAtPoint(worldX, worldY);
            }
        });
         if(this.inputHandler) this.inputHandler.updateMouseCursor();
    }
    ...
    initializeNewCampaign() {
        ...
        this.selectedUnits = [];
        this.tempSelectedForDeployment = [];
        this.preloadedImages = {}; // Clear preloaded images for a new campaign
        ...
        for (let i = 0; i < initialSize; i++) {
            let faceImageFile = 'default_face.png'; // Fallback
    ...
    getAvailableRecruits() { return this.masterRoster.filter(r => r.isAlive()); }
    ...
    setFormationSpacing(multiplier) { if (this.gameState === 'RUNNING') this.formationSpacingMultiplier = parseFloat(multiplier); }
    ...
    selectUnitsInDragRectangle() {
        ...
        if(this.deployedSquadRoster) this.deployedSquadRoster.forEach(unit => {
            if (unit.isAlive() && unit.x >= rectX && unit.x <= rectX + rectWidth && unit.y >= rectY && unit.y <= rectY + rectHeight) {
    ...
    addProjectile(projectile) { this.gameObjects.push(projectile); }
    ...
    checkMissionStatus() {
        ...
        if (this.currentMissionParams && this.currentMissionParams.objectiveType === 'EXTERMINATE') {
            ...
            if (this.initialEnemyCount === 0 && (!this.enemyUnits || this.enemyUnits.length === 0)) this.isObjectiveComplete = true; // No enemies to kill
    ...
    update(deltaTime) {
        ...
        allUnitsInGame.forEach(unit => { if (unit) unit.update(deltaTime); });
    ...
    render() {
        ...
        this.ctx.fillStyle = '#385434'; this.ctx.fillRect(0, 0, CONFIG.WORLD_WIDTH || 0, CONFIG.WORLD_HEIGHT || 0);
        if(this.level.obstacles) this.level.obstacles.forEach(obstacle => {
             if (obstacle.isDestroyed && obstacle.imageDestroyed) { // For destroyable sprites
                this.ctx.drawImage(obstacle.imageDestroyed, obstacle.x, obstacle.y, obstacle.width, obstacle.height);
            } else if (!obstacle.isDestroyed && obstacle.imageNormal) { // For normal state or non-destroyable sprites like grass
                this.ctx.drawImage(obstacle.imageNormal, obstacle.x, obstacle.y, obstacle.width, obstacle.height);
            } else { // Fallback to color if no image or image failed to load
                let obsColor = obstacle.color || '#555555';
                if (obstacle.isDestroyed) { // For non-sprite obstacles that are destroyed
                    obsColor = 'rgba(50, 40, 30, 0.7)';
                } else if (obstacle.destructible && obstacle.hp < obstacle.maxHp && obstacle.hp > 0) {
                    // ... (damaged color logic from before) ...
                    const damageRatio = Math.max(0, obstacle.hp / obstacle.maxHp); let r = parseInt(obsColor.substring(1,3),16), g = parseInt(obsColor.substring(3,5),16), b = parseInt(obsColor.substring(5,7),16);
                    r = Math.floor(r*damageRatio+80*(1-damageRatio)); g = Math.floor(g*damageRatio+80*(1-damageRatio)); b = Math.floor(b*damageRatio+80*(1-damageRatio));
                    obsColor = `rgb(${r},${g},${b})`;
                }
                // Only draw color rect if it's NOT a decoration that failed to load its sprite
                // (or if you want a fallback color for missing decoration sprites)
                if (!obstacle.isDecoration || !obstacle.imageNormal) { // Don't draw color for grass if its sprite is just missing
                    this.ctx.fillStyle = obsColor;
                    this.ctx.fillRect(obstacle.x, obstacle.y, obstacle.width, obstacle.height);
                }
            }
            // HP bar for damaged destructibles (regardless of sprite/color)
            if (obstacle.destructible && !obstacle.isDestroyed && obstacle.hp < obstacle.maxHp && obstacle.hp > 0) {
                const hpBarHeight = 6; const hpBarWidth = Math.min(obstacle.width * 0.7, 60); const barX = obstacle.x + (obstacle.width - hpBarWidth) / 2; const barY = obstacle.y - hpBarHeight - 4;
                this.ctx.fillStyle = '#111'; this.ctx.fillRect(barX - 1, barY - 1, hpBarWidth + 2, hpBarHeight + 2);
                this.ctx.fillStyle = '#c00'; this.ctx.fillRect(barX, barY, hpBarWidth, hpBarHeight);
                this.ctx.fillStyle = '#0c0'; this.ctx.fillRect(barX, barY, hpBarWidth * (obstacle.hp / obstacle.maxHp), hpBarHeight);
            }
        });
        this.visualEffects.forEach(effect => { if (effect && effect.type === 'explosion_ground_mark') effect.render(this.ctx); });
        const unitsToRenderOnMap = [...(this.deployedSquadRoster || []), ...(this.enemyUnits || [])];
        unitsToRenderOnMap.forEach(unit => { if (unit) unit.render(this.ctx); });
        this.gameObjects.forEach(obj => { if (obj) obj.render(this.ctx); });
        this.visualEffects.forEach(effect => { if (effect && effect.type !== 'explosion_ground_mark') effect.render(this.ctx); });
        ...
        if (aimingRaccoon && this.inputHandler && this.inputHandler.mousePos) {
            ...
            if (throwDist > CONFIG.RACCOON_GRENADE_THROW_RANGE_MAX) {
                 const angle = Math.atan2(worldMouseY - aimingRaccoon.y, worldMouseX - aimingRaccoon.x); const cappedX = aimingRaccoon.x + Math.cos(angle) * CONFIG.RACCOON_GRENADE_THROW_RANGE_MAX; const cappedY = aimingRaccoon.y + Math.sin(angle) * CONFIG.RACCOON_GRENADE_THROW_RANGE_MAX;
