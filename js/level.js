...
class Level {
    ...
    generateLevelAndGetPlayerSpawns(worldWidth, worldHeight, missionParamsContainer = {}, numPlayerSpawnsNeeded, preloadedAssetImages = {}, missionSeed) {
        ...
        const maxGenerationAttempts = 10; // Safety break
        let finalPlayerSpawnLocations = [];

        do {
            generationAttempts++;
            if (generationAttempts > 1) {
                console.warn(`[Level Gen] Level validation failed. Retrying... (Attempt ${generationAttempts})`);
                // Use a new seed derived from the original to get a different layout
                missionSeed = missionSeed + generationAttempts * 1337;
            }
            ...
            const effectiveSpawnZoneY = (playerSpawnZone.y + playerSpawnZone.height) - spawnAreaHeight - fixedPadding;
            const effectiveSpawnZoneWidth = spawnAreaWidth; 
            const effectiveSpawnZoneHeight = spawnAreaHeight;
            
            this.effectivePlayerSpawnZone = {
            ...
            for (let i = 0; i < numPlayerSpawnsNeeded; i++) {
                ...
                if (effectiveSpawnZoneWidth > playerUnitSize && effectiveSpawnZoneHeight > playerUnitSize) {
                    do {
                        ...
                        if (isClear) { finalPlayerSpawnLocations.push({ x: spawnX, y: spawnY }); foundSpot = true; break; } 
                ...
                if (!foundSpot) {
                    ...
                    finalPlayerSpawnLocations.push({ x: spawnX, y: spawnY });
                }
            }

            // --- VALIDATION LOGIC ---
            const navGrid = this.getNavigationGrid();
            if (!navGrid) {
                console.error("[Level Gen] CRITICAL: NavGrid failed to generate. Level is invalid.");
                isLevelValid = false;
            } else {
                const anchorPoints = [];
                // 1. Player Spawn Anchor
                const playerSpawnAnchor = this.worldToGridCoords(playerSpawnZone.x + playerSpawnZone.width / 2, playerSpawnZone.y + playerSpawnZone.height / 2);
                anchorPoints.push({ name: 'PlayerSpawn', ...playerSpawnAnchor });

                // 2. Objective Anchors
                missionObjectives.forEach(obj => {
                    if (obj.isPrimary) {
                        if (obj.type === 'DESTROY_TARGET') {
                            const targets = this.obstacles.filter(o => o.isMissionTarget && o.objectiveId === obj.id);
                            targets.forEach(t => anchorPoints.push({ name: `Objective_${t.type}`, ...this.worldToGridCoords(t.x + t.width / 2, t.y + t.height / 2) }));
                        } else if (obj.type === 'ASSASSINATION' && obj.targetUnitId) {
                            const targetUnit = this.game.enemyUnits.find(u => u.id === obj.targetUnitId);
                            if (targetUnit) anchorPoints.push({ name: 'Objective_VIP', ...this.worldToGridCoords(targetUnit.x, targetUnit.y) });
                        }
                    }
                });

                // 3. Extraction Zone Anchor
                const evacZone = this.obstacles.find(o => o.type === 'extraction_zone');
                if (evacZone) {
                    anchorPoints.push({ name: 'EvacZone', ...this.worldToGridCoords(evacZone.x + evacZone.width / 2, evacZone.y + evacZone.height / 2) });
                }

                // 4. Run the checks
                let allPathsExist = true;
                const startPoint = anchorPoints[0];
                for (let i = 1; i < anchorPoints.length; i++) {
                    const endPoint = anchorPoints[i];
                    const path = findPath(startPoint, endPoint, navGrid, false);
                    if (!path || path.length === 0) {
                        console.warn(`[Level Validation] FAILED: No path from ${startPoint.name} to ${endPoint.name} at (${endPoint.x}, ${endPoint.y})`);
                        allPathsExist = false;
                        break;
                    }
                }
                isLevelValid = allPathsExist;
            }

        } while (!isLevelValid && generationAttempts < maxGenerationAttempts);

        if (!isLevelValid) {
            console.error(`[Level Gen] CRITICAL: Failed to generate a valid level after ${maxGenerationAttempts} attempts. Aborting.`);
            // In a real game, you might load a fallback level or show an error screen.
            // For now, we'll proceed with the last (potentially broken) level.
        }

        return finalPlayerSpawnLocations;
    }
