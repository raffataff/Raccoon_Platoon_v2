// js/possumEliteGuard.js

class PossumEliteGuard extends Unit {
    constructor(x, y, game, id) {
        super(x, y, game, 'enemy',
              CONFIG.POSSUM_ELITE_GUARD_HP,
              CONFIG.POSSUM_ELITE_GUARD_SPEED,
              CONFIG.POSSUM_ELITE_GUARD_SIZE,
              CONFIG.POSSUM_ELITE_GUARD_COLOR,
              id || `ELITEGUARD-${Date.now().toString(36).slice(-4)}`);

        this.turnRate = CONFIG.POSSUM_ELITE_GUARD_TURN_RATE;
        this.deadSpritePathKey = 'POSSUM_ELITE_GUARD_DEAD_SPRITE_PATH';
        this.deadSpriteFilesKey = 'POSSUM_ELITE_GUARD_DEAD_SPRITE_FILES';
        this.deadSpriteScaleKey = 'POSSUM_ELITE_GUARD_DEAD_SPRITE_SCALE';

        this.weaponName = CONFIG.POSSUM_ELITE_GUARD_DEFAULT_WEAPON || 'POSSUM_ELITE_GUARD_WEAPON';
        this.canShootWhileMoving = false;

        this.bossAIConfig = CONFIG.AI.POSSUM_ELITE_GUARD || {};
        this.detectionRange = this.bossAIConfig.DETECTION_RANGE || 550;

        this.aiState = 'GUARDING';
        this.guardPost = { x: x, y: y };
        this.actionTimer = 0;
        this.xpValue = CONFIG.XP_FOR_ELITE_GUARD_KILL || 200;
    ...
    _handleEnemyCombat(deltaTime, obstacles) {
        ...
        if (needsChaseDestUpdate) {
            ...
            const predictionTime = this.bossAIConfig.CHASE_PREDICTION_TIME_FACTOR || 0.30;
